...
#### Situação de emprego x Expectativa após graduação {.tabset .tabset-fade .tabset-pills}

```{r, comment=FALSE, message=FALSE, fig.align='center', warning=TRUE, echo = FALSE}

temp <- data.frame(emprego = quest_23$emprego,
                   expectativa = quest_23$expectativa_pos_graduacao)


levels(temp$emprego) <- c("Não",
                   "Vínc.Formal", 
                   "Trab.Ocasional", 
                   "Trab.Contrato", 
                   "Estágio",
...
levels(temp$expectativa) <- c("Conseguir Emprego", 
                   "Aumento salarial", 
                   "Seguir os estudos", 
                   "Outro")
...
table2 <- as.matrix(prop.table(table(quest_23$expectativa_pos_graduacao,
                                     quest_23$emprego), 
                               mar = 2))
...
#### Sexo x Número de quadrados na figura

```{r, comment=FALSE, message=FALSE, fig.align='center', warning=TRUE, echo = FALSE, warning=FALSE, fig.width=10}
ggplot(data = quest_23, 
       mapping = aes(x = sexo,
                     y=quadrados)) +
  ...
  xlab("Sexo") +
  ylab("Quadrados") +
  ggtitle("Sexo x Número de quadrados na figura")+
...
```{r, comment=FALSE, message=FALSE, fig.align='center', warning=TRUE, echo = FALSE, warning=FALSE, fig.width=10}
ggplot(data = subset(quest_23, peso >=15 & 
                       peso <=200 & 
                       altura >=90 & 
                       altura <=250), 
       mapping = aes(x = peso,
...
```{r, comment=FALSE, message=FALSE, fig.align='center', warning=TRUE, echo = FALSE, warning=FALSE, fig.width=10}
ggplot(data = subset(quest_23, idade >=15 & 
                       idade <=100 & 
                       atv_fisica_n >=0 & 
                       atv_fisica_n <=7), 
       mapping = aes(x = idade,
